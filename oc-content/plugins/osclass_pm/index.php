<?php
/*
Plugin Name: Personal Messaging for OSClass wefwerfwefwefrgefg sdfgdsfgsf
Plugin URI:
...
// Before HTML
   ...
   function osclass_pm_before_html() {
      ...
      if(osc_is_web_user_logged_in() && pmFlashAlert() == 1 && !osclass_pm_is_inbox() && !osclass_pm_is_messages() ) {
         ...
         if($countPMs > 0 ) {
			 osc_add_flash_ok_message('<a href="'.osc_render_file_url(osc_plugin_folder(__FILE__) . 'user-inbox.php').'" >'.sprintf(_n('You have %d new Personal Message!', 'You have %d new Personal Messages!', $countPMs, 'osclass_pm'), $countPMs).'</a>');
         }
      }
   }
...
// Everything between this section is user side related.
    ...
    */ ?>
    ...
    function osclass_pm_link($class = 'btn') {
		if(osc_is_ad_page() && osc_is_web_user_logged_in() && osc_item_user_id() != ''){
          $user = User::newInstance()->findByPrimaryKey(osc_item_user_id());
			echo '<a class="' . $class . '" href="' . osc_base_url(true) . '?page=custom&file=osclass_pm/user-send.php&userId=' . osc_item_user_id() . '&itemId=' . osc_item_id() . '&mType=new">' . __('Send PM to ','osclass_pm') . $user['s_name'] . '</a>';
       } elseif(osc_is_ad_page() && !osc_is_web_user_logged_in()) {
          $user = User::newInstance()->findByPrimaryKey(osc_item_user_id());
       ...
       elseif(osclass_pm_is_pub_profile() && osc_is_web_user_logged_in()){
          $userId = Params::getParam('id');
          $user = User::newInstance()->findByPrimaryKey($userId);
	    echo '<a class="' . $class . '" href="' . osc_base_url(true) . '?page=custom&file=osclass_pm/user-send.php&userId=' . $userId  . '&itemId=' . '&mType=new">' . __('Send PM to ','osclass_pm') . $user['s_name'] . '</a>';	
	   } 
elseif(osclass_pm_is_pub_profile() && !osc_is_web_user_logged_in()) {
          $userId = Params::getParam('id');
          $user = User::newInstance()->findByPrimaryKey($userId);
            echo '<a class="' . $class . '" href="' . osc_user_login_url() . '&http_referer=' . osc_base_url(true) . '?page=custom&file=osclass_pm/user-send.php&userId=' . $userId . '&itemId=' . '&mType=new">' . __('Login to contact seller.','osclass_pm') . '</a>';
	}  
	}
